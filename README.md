...
## Dashboard (browser se dekhne k liye)

Ek simple password-protected dashboard bhi hai jahan se orders aur unka status dekh sakte ho, aur ek button se bulk WhatsApp confirmation bhej sakte ho:

```
https://YOUR-RENDER-URL.onrender.com/dashboard
```

Browser login popup m:
- **Username**: kuch bhi likh do (e.g. `admin`)
- **Password**: wahi `ADMIN_API_KEY` jo `.env` / Render environment m set kiya tha
