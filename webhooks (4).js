...
router.post('/orders-create', async (req, res) => {
  ...
  try {
    ...
    // Orders created manually from the Shopify Admin (e.g. clicking "Create order",
    // recreating a previously deleted order, intentional duplicates) are NEVER
    // fraud/duplicate checked or auto-cancelled, no matter how many times you make
    // them. Only real customer storefront checkouts (source_name === 'web') go
    // through the fraud/duplicate evaluation below.
    if (order.source_name && order.source_name !== 'web') {
      await upsertOrder(toDbRow(order, 'processing', true));
      await logEvent(order.id, 'admin_created_order_skipped_fraud_check', order.source_name);
      return;
    }
...
// ---- 5) orders/delete -> mark local record cancelled so it stops counting as an
// "active order in process" and no longer wrongly blocks a legit re-created order ----
router.post('/orders-delete', async (req, res) => {
  ...
  try {
    const order = parseOrderFromRawBody(req);
    await setStatus(String(order.id), 'cancelled');
    await logEvent(order.id, 'order_deleted_in_shopify', 'marked cancelled locally so it no longer blocks future orders as "in process"');
  } catch (err) {
    console.error('orders-delete webhook error:', err.message);
  }
});
